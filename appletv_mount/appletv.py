...
def get_height(tolerance = 0.0):
    return 31 + 2*tolerance
    
def get_base_size():
    return 93.24

def render(tolerance = 0.0):
    ...
    # -----------------------------
    base_size = get_base_size()     # total square side length
    height = get_height(tolerance)          # final extrude thickness
    ...
    # place that wire into a document object
    full_wire_obj = doc.addObject("Part::Feature", "AppleTV_Perimeter_Wire")
    full_wire_obj.Label = "AppleTV_Perimeter_Wire"
    full_wire_obj.Shape = full_wire
    doc.recompute()
    # offset it to account for tolerances
    offset_obj = doc.addObject("Part::Offset2D", "AppleTV_1D_Offset")
    offset_obj.Source = full_wire_obj
    offset_obj.Value = tolerance
    doc.recompute()
    offset_obj_copy = doc.addObject("Part::Feature", "AppleTV_1D")
    offset_obj_copy.Shape = offset_obj.Shape.copy()
    doc.recompute()
    # use that to create a face
    full_face = Part.Face(offset_obj.Shape)
    full_face.fix(1e-6, 1e-6, 1e-6)
    full_face_obj = doc.addObject("Part::Feature", "AppleTV_2D")
    ...
    offset_obj_copy.ViewObject.Visibility = False
    full_face_obj.ViewObject.Visibility = False
    solid_obj.ViewObject.Visibility = False
    full_wire_obj.ViewObject.Visibility = False
